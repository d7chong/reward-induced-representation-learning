...
class MovingSpriteDataset(Dataset):
    ...
    def __len__(self):
        return self._spec.num_trajectories

    def __getitem__(self, item):
        traj = self._generator.gen_trajectory()

        data_dict = AttrDict()
        #data_dict.images = traj.images[:, None].repeat(1, axis=3).astype(np.float32) / (255./2) - 1.0
        data_dict.images = traj.images[:, None].astype(np.float32) / 255
...
class MovingSpritesGenerator:
    ...
    def gen_trajectory(self):
        ...
        output.states = self._traj_gen.create(self._spec.T, self._spec.shapes_per_traj)
